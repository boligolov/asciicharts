# asciicharts.online — the landing page
...
`Base.astro` emits a canonical URL plus Open Graph and Twitter card tags, so a link pasted into Slack,
Telegram, X or LinkedIn unfurls with a title, description and a 1200×630 image. Those need absolute
URLs, built from `site` in `astro.config.mjs` (`https://asciicharts.online`) — change it if the site is
served elsewhere.

The image, `public/og.png`, is a screenshot of `src/pages/og.astro`: the same fonts, logo and real
`render_chart()` output as the page (the hero chart, in color), not a mockup. After changing the card
or the hero chart, re-shoot it (needs Playwright with Chromium) and commit the PNG:

```sh
npm run dev                          # in site/, in another terminal
python scripts/og_image.py           # from the repository root; writes site/public/og.png
```
...
src/
  ...
  pages/og.astro           the 1200×630 link-preview card (noindex), screenshotted into public/og.png
```
