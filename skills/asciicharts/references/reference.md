...
| `thresholds`| array             | line                                  | several reference lines, each a number or `{"value": 106, "label": "target"}`; each is dashed and named with its value right of the plot (the label alone when it already states the value, like `"5%"`). A reference line is drawn behind the data: where a series runs along it, the series shows and the label on the right still names the line. At most 20, labels up to 40 characters |
