...
- **The `░` behind a bar is normal — it's the rest of the axis, not noise.** hbar/vbar in the default `solid`/`fine` style shade the unused part of each bar up to the chart's own 0..max scale, like a progress-bar track, so a short bar's real size reads at a glance. It's automatic — nothing to set — and absent from stacked/diverging bars, `halftone`, `ascii` and histograms.
